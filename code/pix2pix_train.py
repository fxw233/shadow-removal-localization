...
for epoch in range(args.epoch_start, args.epoch_num):
    for i, batch in enumerate(train_dataloader):
        ...
        # Pixel-wise loss
        sigmoid = nn.Sigmoid()
        loss_pixel = criterion_pixelwise(sigmoid(fake_B), real_B)
    ...
    if args.checkpoint_interval != -1 and epoch  == 199:
        # Save model checkpoints
        torch.save(generator.state_dict(), 'result/generator2_%d.pth' % epoch)
        torch.save(discriminator.state_dict(), 'result/discriminator2_%d.pth' % epoch)
